...
* 想让Vue工作，就必须创建一个Vue实例，且要传入一个option对象
...
https://www.cnblogs.com/ella-li/p/14747201.html
...
* 事件的回调需要配置在methods对象中，最终会添加到vm(vue根实例)上
...
<script>
	const vm = new Vue({
        ...
        methods:{
            // 如果vue模板没有写$event，会自动传 $event 给函数
            showInfo1(event){
...
* 退出 => esc
* 空格 => space
* 换行 => tab (特殊，必须配合keydown去使用)timeFormat
...
<<<<<<< HEAD
* 定义：要用的属性不存在，要通过已有属性（注意不是普通变量）计算得来
=======
* 定义：要用的属性不存在，要通过已有属性(注意不是普通变量)计算得来
>>>>>>> 86bda0448fb6fa9f584dd40f176f37cf6a44c214
* 原理：底层借助了Object.defineProperty方法提供的getter和setter函数
* 注意：计算属性默认只有 getter
* get函数什么时候调用执行？
  * (1).初次读取计算属性时，会调用执行get函数。 注意：Vue模版多次使用计算属性，如果所依赖的数据没有改变，只会调用执行 1 次get函数
  * (2).当计算属性所依赖的属性改变时，会再次调用执行get函数。
      原因：因为计算属性内部有缓存机制，如果所依赖的数据发生变化，还复用缓存的计算属性，显然是错误的。
* 优势：与methods方法实现相比，计算属性内部有缓存机制（复用），效率更高。（原因：**计算属性是基于它们的响应式依赖进行缓存的**，只在相关响应式依赖发生改变时它们才会重新求值，所以只要 `message` 还没有发生改变，多次访问 `reversedMessage` 计算属性会立即返回之前的计算结果，而不用再次执行函数）
* 备注：
  <<<<<<< HEAD
  * 计算属性最终会出现在vm上，直接读取使用即可，不要写成计算属性.get()（vm可以直接读取计算属性的原理：Vue读取计算属性，调用执行get函数，将get函数的返回值挂载到vm上，作为vm的计算属性的值。）
  * 如果修改计算属性或修改计算属性所依赖的属性，那必须写set函数去响应修改（如果不写set函数，会报Vue warn：计算属性被修改赋值，但没有setter函数），而且在set函数中,计算属性所依赖的属性需要改变（如果计算属性所依赖的属性不改变，只是单纯console打印，那set函数没有意义，因为计算属性所依赖的属性没有相应更新）
  =======
  * 计算属性最终会出现在vm上，直接读取使用即可，不要写成计算属性.get()（vm可以直接读取计算属性的原理：Vue读取计算属性，调用执行get函数，将get函数的返回值挂载到vm上，并作为vm的计算属性的值。）
  * 如果修改计算属性，那必须写set函数去响应修改（如果不写set函数，会报Vue warn：计算属性被修改赋值，但没有setter函数），而且在set函数中,计算属性所依赖的数据要改变（如果计算属性所依赖的数据不改变，那set函数没有意义，因为计算属性没有被修改）
>>>>>>> 86bda0448fb6fa9f584dd40f176f37cf6a44c214

注意：计算属性的get函数和set函数是被Vue所管理的函数，不能写成箭头函数，否则this指向Window对象
...
<<<<<<< HEAD
![img](https://img-blog.csdnimg.cn/0acfc64ee54d44418ff0a2750a6c4ffd.png?x-oss-process=image/watermark,type_d3F5LXplbmhlaQ,shadow_50,text_Q1NETiBA5qC86Zu354uQ5oCd,size_17,color_FFFFFF,t_70,g_se,x_16)
=======
...
> 计算属性简写版写法：提供的函数fullName将用作计算属性vm.fullName的 getter 函数
...
* 当被侦听的属性变化时, 自动调用回调函数, 进行相关操作
* 侦听的属性（可以是data属性或computed属性）必须存在，才能进行侦听
* 侦听的两种写法：
  * (1).new Vue时传入watch选项配置对象，注意侦听属性有简写版和完整版，指的是函数式和对象式  -- 更推荐
  * (2).通过实例方法：vm.$watch侦听，传入的参数可以是函数或对象
...
<script>
	const vm = new Vue({
        ...
        watch:{
            isHot:{
                immediate: true, // 初始化时让handler立即调用一下，立即侦听，不管数据是否有变化，上来立即侦听一次
...
<script>
    ...
    vm.$watch('isHot',{
        immediate:true, //初始化时让handler立即调用一下
        //handler什么时候调用？当isHot发生改变时。
        handler(newValue,oldValue){
            console.log('isHot被修改了',newValue,oldValue)
        }
...
* (2).配置deep:true可以侦听对象内部值的改变（多层）



> 备注：
>
> (1).Vue自身可以侦听data对象内部值的改变，但Vue提供的watch默认不可以
>
...
key是虚拟DOM的标识，当数据发生变化时，Vue会根据【新数据】生成【新的虚拟DOM】后，将【新虚拟DOM】与【旧虚拟DOM】进行差异（diff）比较，比较规则如下：

* 旧虚拟DOM中找到了与新虚拟DOM相同的key：
  * ①.若虚拟DOM中内容没变, 直接复用/使用之前的真实DOM！
  * ②.若虚拟DOM中内容变了, 则生成新的真实DOM后，替换掉页面中之前的真实DOM。
...
会产生没有必要的真实DOM更新 ==> 界面显示效果没问题, 但效率低。
...
在 persons 数组开头添加上 { id: '004', name: '老刘', age: 40 }
...
![img](https://img-blog.csdnimg.cn/ec80a7ae139642d09d70e77ca37e0a52.png?x-oss-process=image/watermark,type_d3F5LXplbmhlaQ,shadow_50,text_Q1NETiBA5qC86Zu354uQ5oCd,size_20,color_FFFFFF,t_70,g_se,x_16)

**如果结构中还包含输入类的DOM：**

会产生错误DOM更新 ==> 界面有问题。

> 这回造成的就不是性能浪费了，会直接导致页面的显示错误
...
* 如果不存在对数据的逆序添加、逆序删除等破坏数据顺序的操作，仅用于渲染列表用于展示，使用index作为key是没有问题的



> 来张尚硅谷的图，正经使用 key（id作为key）
...
> 讲一下解析模板后面的操作---》调用 set 方法时，就会去解析模板----->生成新的虚拟 DOM----->新旧虚拟DOM 对比 -----> 更新页面
...
向响应式对象中添加一个 property，并确保这个新 property 同样是响应式的，且触发视图更新。它必须用于向响应式对象上添加新 property，因为 Vue 无法监测普通的新增 property (比如 `vm.myObject.newProperty = 'hi'`)
...
既然 vue 在对数组无法通过 getter 和 setter 进行数据侦听，那 vue 到底如何侦听数组数据的变化呢？
...
* 如何监测对象中的数据？
  ...
  通过setter实现侦听，且要在new Vue时就传入要监测的数据。
...
> 特别注意：Vue.set() 和 vm.$set() 不能给vm 实例或 vm实例的根数据对象$data 添加响应式属性！！！



## 1.15 收集表单数据

若：input输入表单元素的type为text/password/number，则v-model收集的是value值，用户输入的就是value值。
...
配置对象中常用的3个回调函数，或者叫钩子函数：

* bind：当指令第一次绑定到元素时，调用bind钩子函数。
* inserted：当指令所绑定的元素插入到 DOM 中时，调用inserted钩子函数。
* update：指令所在模板被重新解析时调用，或者说元素更新时调用。

> 理解这三个的调用时机，需要进一步了解 vue 的生命周期，下面会介绍。

注意：1.指令定义时不加-v，但使用时要加v-

​			2.指令名如果是多个单词，要使用连接符，不要用小驼峰命名
...
Vue 实例有⼀个完整的⽣命周期，从创建Vue实例(new Vue())、初始化事件(.once事件)和生命周期、编译模版、挂载Dom -> 渲染、更新 -> 渲染、卸载 等⼀系列过程，称这是Vue的⽣命周期。 

先来一张尚硅谷的图：

<img src="https://img-blog.csdnimg.cn/e4a8169c832443f2a78c6a1ae42a87b3.png?x-oss-process=image/watermark,type_d3F5LXplbmhlaQ,shadow_50,text_Q1NETiBA5qC86Zu354uQ5oCd,size_20,color_FFFFFF,t_70,g_se,x_16" alt="在这里插入图片描述"  />
...
2. **created（创建后）**：vm或组件实例创建完成，实例上配置的 options 包括 data、computed、watch、methods 等都配置完成，可以通过实例访问data属性和methods方法等。但是，此时渲染的节点还未挂载到 DOM，所以不能访问到 `$el`属性。
3. **beforeMount（挂载前）**：在挂载开始之前被调用，相关的render函数首次被调用。此阶段Vue开始编译解析模板，内存中生成虚拟DOM，但没有将虚拟DOM转为真实DOM，真实DOM未被插入页面。所以网页不能显示解析好的内容。
4. **mounted（挂载后）**：创建的 vm.$el用于生成的真实DOM（vm.$el是真实DOM的拷贝）会替换el选项对应的DOM，并将vm实例挂载到DOM后（注意这个时间节点已完成的功能：将内存中的虚拟DOM转为真实DOM， 真实DOM被插入页面），开始调用生命周期钩子函数mounted 。挂载后，页面中呈现的是经过Vue编译的DOM，这时在这个钩子函数中对DOM的操作可以有效，但要尽量避免。一般在这个阶段进行：开启定时器，发送网络请求，订阅消息，绑定自定义事件等等
5. **beforeUpdate（更新前）**：响应式数据更新时调用。此时虽然响应式数据更新了，但是对应的真实 DOM 还没有被重新渲染和打补丁（数据是新的，但页面是旧的，页面和数据还没保持同步）。
6. **updated（更新后）** ：当响应式数据更新，生成新的虚拟DOM，与旧的虚拟DOM对比，重新渲染和打补丁后，调用生命周期钩子函数updated。注意此时DOM模板已经根据响应式数据的变化更新了。调用时，组件DOM模板已经更新，所以可以执行依赖于DOM的操作。然而在大多数情况下，应该避免在此期间更改状态，因为这可能会导致更新无限循环。该钩子在服务端渲染期间不被调用。（数据是新的，页面也是新的，页面和数据保持同步）
7. **beforeDestroy（销毁前）**：实例销毁前调用。这一步，实例仍然完全可用，`this` 仍能获取到实例。在这个阶段一般进行关闭定时器，取消订阅消息，解绑自定义事件。
8. **destroyed（销毁后）**：实例销毁后调用。调用生命周期钩子函数destroyed后，Vue 实例指示的所有东西都会解绑，所有的事件监听器会被移除，所有的子实例也会被销毁。该钩子在服务端渲染期间不被调用。
...
​	3.一般不会在beforeDestroy操作数据，因为即便操作数据，也不会触发更新数据了

> 来讲一下图中间大框框的内容

![img](https://img-blog.csdnimg.cn/bf970362485848798ab093430b4162b8.png?x-oss-process=image/watermark,type_d3F5LXplbmhlaQ,shadow_50,text_Q1NETiBA5qC86Zu354uQ5oCd,size_20,color_FFFFFF,t_70,g_se,x_16)

先判断有没有 **el** 配置项，没有就调用 vm.$mount(el)，如果两个都没有就一直卡着，显示的界面就是最原始的容器的界面。有 **el** 这个配置项，就进行判断有没有 template 这个配置项，没有 template 就将 el 绑定的容器编译解析为 vue 模板，来个对比图。
...
![img](https://img-blog.csdnimg.cn/2c59b4bb3820404f87adf366e8582f91.png?x-oss-process=image/watermark,type_d3F5LXplbmhlaQ,shadow_50,text_Q1NETiBA5qC86Zu354uQ5oCd,size_20,color_FFFFFF,t_70,g_se,x_16)
...
![img](https://img-blog.csdnimg.cn/23762cb8feb340449fb0033abcff0a6d.png?x-oss-process=image/watermark,type_d3F5LXplbmhlaQ,shadow_50,text_Q1NETiBA5qC86Zu354uQ5oCd,size_20,color_FFFFFF,t_70,g_se,x_16)



**第二种情况，没有 template：**   注意：工程化一般不用template选项，直接使用el: '#app'或vm.$mount('#app')，将index.html的<div id="app"></div>编译解析为Vue模板。index.html的<div id="app"></div>是el绑定的容器
...
关于组件名：  注意：这里的组件名和option对象中name配置项不一样

一个单词组成：

* 第一种写法(首字母小写)：school   推荐使用
* 第二种写法(首字母大写)：School

多个单词组成：

* 第一种写法(kebab-case命名)：my-school  推荐使用
...
> 备注：不使用vue-cli脚手架时，<school/>会导致后续组件不能渲染。
...
* 为什么要有这个关系：让组件实例对象（vc）可以访问到 Vue原型对象上的属性、方法。
...
单文件组件就是将一个组件的代码写在 .vue 这种格式的文件中，webpack 会将 .vue 文件解析成 html,css,js这些格式文件。
...
从上面的小知识可以知道，我们引入的 vue 不是完整版的，是残缺的（为了减小vue.js文的文件大小）。所以残缺的vue.js 只有通过 render 函数才能把项目给跑起来。
...
功能：用于增强Vue。举个游戏例子：首先开好锁头挂，再打CF，这里的锁头挂就是Vue的插件，CF就是Vue

插件通常用来为 Vue 添加**全局**功能。插件的功能范围没有严格的限制。
...
3. 注意：深度选择器，即使父组件加了scoped，也可以影响子组件的样式

   <span style="color:red">>>></span>  一般用于原生css
...
```
 ...
 报错信息：Can't resolve 'less-loader'

 解决方案：npm i less-loader

 如果webpack的版本和less-loader版本不兼容，导致无法安装
...
组件的自定义事件是一种组件间通信的方式，适用于：<strong style="color:red">子组件 ===> 父组件</strong>
...
3. 使用事件总线：
   ...
   1. 接收数据：A组件想接收数据，则在A组件中给$bus绑定或监听自定义事件，<span style="color:red">事件的回调写在A组件自身 。</span>
   ...
   2. 提供数据：B组件或其它组件触发自定义事件```this.$bus.$emit('xxxx',数据)```

4.  最好在beforeDestroy钩子中，用$off去解绑<span style="color:red">当前组件A所用到的</span>自定义事件。
...
2. 作用：在下次 DOM 更新循环结束后执行延迟回调。
3. 什么时候用：当修改数据后，要基于更新后的新DOM进行某些操作时调用。要在$nextTick所指定的延迟回调中执行某些操作。





## 2.8 Vue封装的过渡与动画，使用内置组件transition或transition-group

​	注意：组件标签或元素必须要有**v-if或v-show**条件渲染指令，才可以进行过渡与动画
...
2. 写法：
   ...
   1. 准备好样式：
      ...
      - 元素进入的样式：
        1. v-enter：进入的起点，即过渡的开始状态（进入）
        2. v-enter-active：进入过程中，比如可以定义过渡的花费时间和运动曲线
        3. v-enter-to：进入的终点，即过渡的结束状态（进入）
      - 元素离开的样式：
        1. v-leave：离开的起点
        2. v-leave-active：离开过程中
        3. v-leave-to：离开的终点

   2. 使用```<transition>```内置组件包裹要过渡的元素，并配置name属性：
...
1. 作用：让父组件可以向子组件<strong style="color:red">slot出口</strong>插入html结构，也是一种组件间通信的方式，适用于 <strong style="color:red">父组件 ===> 子组件</strong> 。
...
3. 使用方式：
   ...
   3. 作用域插槽：
      ...
      1. 理解：<span style="color:red">数据在定义插槽的组件自身，但根据数据生成的结构需要组件的使用者来决定。</span>（games数据在Category组件中，但使用数据所遍历出来的结构由App组件决定）
...
4. 组件通信方式汇总
   ...
   ![组件通信方式](C:\Users\xiaobin\Desktop\组件通信方式.png)

​		![组件通信4](C:\Users\xiaobin\Desktop\组件通信4.png)

![组件通信3](C:\Users\xiaobin\Desktop\组件通信3.png)

![组件通信2](C:\Users\xiaobin\Desktop\组件通信2.png)
...
1. 概念：当state中的数据需要经过加工简化后再使用时，可以使用getters加工简化数据。（getters是为了简化数据而生！！！）

2. 在```store.js```中追加```getters```配置

   ```js
   ......
   // 准备getters对象，用于将state对象中的数据进行加工简化
   const getters = {
   	bigSum(state){
   		return state.sum * 10
   	}
...
3. <strong>mapActions方法：</strong>用于帮助我们生成与```actions```对话的方法，即：封装包含```$store.dispatch(xxx)```的函数
...
4. <strong>mapMutations方法：</strong>用于帮助我们生成与```mutations```对话的方法，即：封装包含```$store.commit(xxx)```的函数
   ...
   methods:{
       ...
       //接祖mapMutations生成：JIA、JIAN（数组形式）
       ...mapMutations(['JIA','JIAN']),
   }
   ```

> 备注：mapActions与mapMutations使用时，若需要传递参数：在模板中绑定事件时传递好参数，否则参数是默认事件对象。
...
4. 开启命名空间后，组件中读取getters数据：
   ...
   //方式二：借助mapGetters读取，数组写法
   ...mapGetters('countAbout',['bigSum'])
   //方式三：借助mapGetters读取，对象写法
...
5. 开启命名空间后，组件中调用dispatch
   ...
   //方式二：借助mapActions生成与actions对话的方法：jiaOdd、jiaWait（数组写法）
   ...mapActions('countAbout', ['jiaOdd','jiaWait'])
   //方式三：借助mapActions生成与actions对话的方法：incrementOdd、incrementWait（对象写法）
...
6. 开启命名空间后，组件中调用commit
   ...
   //方式二：借助mapMutations生成与mutations对话的方法：increment、increment（数组写法）
   ...mapMutations('countAbout',['jiaOdd','jiaWait']),
   //方式三：借助mapMutations生成与mutations对话的方法：increment、increment（对象写法）
...
​    路由通俗记忆：根据你的路径(path)，由我（router）决定显示哪个组件(component)

1. 理解： 一个路由（route）就是一组映射关系（key - value），多个路由需要路由器（router）进行管理。

2. 前端路由：key是路径，value是组件。

3. 后端路由：key是路径，value是function，用户处理客户端发送的请求，流程是：服务器接收到一个请求时，根据请求路径找到匹配的函数来处理请求，返回响应数据，比如node.js的value是function

   ```nodejs
   app.get('/students',(request,response){
   	const students = [
   	{id:'001',name:'xingye',age:17},
   	{id:'002',name:'jay',age:18}
   	]
...
```js
{
	...
	//第三种写法：props值为 函数，该函数返回的对象中每一组key-value都会通过props传给Detail组件
	props($route){
		return {
			id:$route.query.id,
...
1. 作用：不借助```<router-link> ```声明式导航实现路由跳转，使用编程式路由导航，让路由跳转更加灵活
...
2. 具体名字：
   1. ```activated```：被 keep-alive 缓存的路由组件被激活时触发。
   2. ```deactivated```：被 keep-alive 缓存的路由组件失活时触发。
...
5. 组件内的守卫：
   ...
   ```js
   //进入守卫：通过路由规则，进入该路由组件前被调用
   beforeRouteEnter (to, from, next) {
   },
   //离开守卫：通过路由规则，离开该路由组件前被调用
...
3. hash模式：
   ...
   1. path路径带#号，不美观 。
   2. 若通过第三方手机App（比如YY直播App里面的好友IM聊天或群聊天）分享地址，若App校验严格，则地址会被标记为不合法(因为App的秩序后台会校验URL是否合法)。
   3. 兼容性较好。

4. history模式：

   1. path路径不带#号，美观 。
   2. history模式的兼容性比hash模式差。（因为history模式底层使用的是HTML5的pushState API，hash模式底层是通过绑定onhashchange事件实现）
   3. 应用部署上线后，需要后端配置支持。因为是单页面应用，只有一个静态资源index.html，如果后台没有正确的配置，当用户在浏览器直接访问 网站比如：`http://oursite.com/user/id` ，没有静态资源叫/user/id，就会返回 404。（解决方案：使用兜底逻辑。在服务端增加一个覆盖所有路由情况的候选资源：如果 URL 匹配不到任何静态资源，则应该返回同一个 `index.html` 页面，这个页面就是你 app 依赖的页面）
